...
//Last changed by: Sam Walz <mail[at]samwalz.com> 20. September 2013
...
/* 
 * functions to be called from the menu-unity player and from the score board-unity player
 */
function PlayerOneControl(number)
{
	if (_gameRunning) return; // security
}

function PlayerTwoControl(number)
{
	if (_gameRunning) return; // security
}

function PlayerThreeControl(number)
{
	if (_gameRunning) return; // security
}

function PlayerFourControl(number)
{
	if (_gameRunning) return; // security
}
...
/*
 * internal functions - not to be called from any unity-player
 */
...
function ShowCountdownToGame()
{
	...
	if (_currentGame) {
		ColorGameCubeActive();
		LoadHTML5Game("countdownBetweenGames.html#5"); // 5 second pause between games
	} else {
		ShowScoreboard();
	}
...
function LoadJavaGame(path)
{
	...
	$("#javaPlayer").append('<object type="application/x-java-applet" height=' + _contentHeight + ' width=' + _contentWidth + '>'
